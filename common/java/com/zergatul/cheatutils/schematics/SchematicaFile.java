...
public class SchematicaFile implements SchemaFile {
    ...
    private static SchematicaFile parse(CompoundTag compound) throws InvalidFormatException {
        ...
        int width = compound.getShort(WIDTH_TAG).orElseThrow();
        int height = compound.getShort(HEIGHT_TAG).orElseThrow();
        int length = compound.getShort(LENGTH_TAG).orElseThrow();
        byte[] blocks = compound.getByteArray(BLOCKS_TAG).orElseThrow();
        byte[] data = compound.getByteArray(DATA_TAG).orElseThrow();
