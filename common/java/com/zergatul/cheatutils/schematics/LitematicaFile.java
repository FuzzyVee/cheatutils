...
public class LitematicaFile implements SchemaFile {
    ...
    private static LitematicaFile parse(CompoundTag compound) throws InvalidFormatException {
        ...
        int version = compound.getInt(VERSION_TAG).orElseThrow();
        int dataVersion = compound.getInt(DATA_VERSION_TAG).orElseThrow();
        ...
        for (String key : regionCompounds.keySet()) {
            regions[index++] = Region.parse(key, regionCompounds.getCompound(key).orElseThrow());
        }
    ...
    private static void validateRequiredTags(CompoundTag compound) throws InvalidFormatException {
        ...
        CompoundTag regions = compound.getCompound(REGIONS_TAG).orElseThrow();
        for (String key : regions.keySet()) {
            validateRegion(regions.getCompoundOrEmpty(key), key);
        }
    }
    ...
    private static void validateRegion(CompoundTag compound, String regionName) throws InvalidFormatException {
        ...
        validateVector(compound.getCompound(POSITION_TAG).orElseThrow(), regionName);
        validateVector(compound.getCompound(SIZE_TAG).orElseThrow(), regionName);
    }
    ...
    private static class Region {
        ...
        public static Region parse(String name, CompoundTag compound) {
            CompoundTag sizeTag = compound.getCompound(SIZE_TAG).orElseThrow();
            int width = Math.abs(sizeTag.getInt(X_TAG).orElseThrow());
